...
import lidc
import metrics
from hyperflow import HyperNet
...
model = HyperNet(net, 1, [24, 24, 24, 24, 24])
model.to(device)
...
for epoch in range(cfg.max_epochs):
    ...
    for images, labels in train_loader:
        ...
        net = model.sample_network(device=device)
        vt = net(args=(t, xt_and_cond))
    ...
    save_model(ema_model, ckpt_dir / "last-hyper.safetensors")
    ...
    net = ema_model.sample_network(device=device)
    sampler = Sampler(net, num_steps=cfg.num_steps)
